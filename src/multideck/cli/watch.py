"""`multideck watch` — the live fleet table. Poll the attention engine,
render most-urgent-first, and jump to a session's window on a digit press.

Complements the ambient signals (`multideck attention`): watch is the
...
if TYPE_CHECKING:
    from multideck.attention import SessionView
...
def _render_frame(views: list[SessionView]) -> None:
    click.echo(
        f"  {style('multideck watch', bold=True)}  "
