...
- Quality gate — **must pass before every commit**: `uv run python scripts/check.py` (or `python scripts/check.py`) runs ruff (curated expanded ruleset in `[tool.ruff.lint]`, linting `src` + `tests` + `scripts`; the frozen `E4/E7/E9/F` baseline stays pinned first, see DESIGN.md for the group-by-group rationale and the per-file-ignore reasons) + `compileall` + mypy (`warn_unused_ignores = true`) + `pytest tests/unit/ --cov=multideck`. CI's `quality` job runs `uv lock --check` then the same gate via `uv sync --extra dev --frozen --python 3.13`; the separate `unit` job runs the OS x Python matrix (3.10–3.14, windows/macos/linux) via plain `pip install -e ".[dev]"`.
