...
process-env MULTIDECK_* var, not merely isolate the dotenv file.

On 2026-07-07 a dev machine's exported MULTIDECK_SENTRY_DSN reached the test
suite and leaked fake unit-test errors to prod Sentry (MULTIDECK-1..4). The
...
The module-scoped fixture below exports MULTIDECK_SENTRY_DSN into the REAL
...
def _exported_sentry_dsn():
    """Simulate `export MULTIDECK_SENTRY_DSN=...` in the operator's shell."""
    previous = os.environ.get("MULTIDECK_SENTRY_DSN")
    os.environ["MULTIDECK_SENTRY_DSN"] = _EXPORTED_DSN
    try:
        yield
    finally:
        if previous is None:
            os.environ.pop("MULTIDECK_SENTRY_DSN", None)
        else:
            os.environ["MULTIDECK_SENTRY_DSN"] = previous


def test_exported_dsn_is_stripped_from_process_env():
    assert "MULTIDECK_SENTRY_DSN" not in os.environ
...
def test_cli_run_does_not_init_sentry_from_exported_dsn(monkeypatch):
    calls: list[str] = []
    monkeypatch.setattr("multideck.sentry.init_sentry", calls.append)
